...
class ChessService {
  ...
  black_move(game) {
    if(!this.checkStatus(game)) {
      const best_black_move = this.makeBestMove(game, 'b');
      return best_black_move
    }
    else {
      return "White-win"
    }
  ...
  makeBestMove(game, color) {
    let move = this.getBestMove(game, color, globalSum)[0];
    globalSum = this.evaluateBoard(game, move, globalSum, 'b');  
    game.move(move);
    const is_continue_game = this.checkStatus(game);
    console.log("is continue game", is_continue_game);
    if(!is_continue_game) {
      return game.fen();
    }
    else {
